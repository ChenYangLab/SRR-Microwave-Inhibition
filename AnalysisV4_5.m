...
uilabel(AccessMenu,'Text',sprintf('%d traces from %s',Range,LoadPath(length(LoadPath)-23:length(LoadPath)-14)),'Position',[5 380 200 15],'FontWeight','Bold');
...
clear ControlLoadedStruct MicrowaveLoadedStruct;
...
% Function that creates a selection UI for the convereted waveform file directory
    ...
    PromptGrid = uigridlayout(PromptFig,[3,1],'RowHeight',{'fit','fit','fit'});
    ...
    uibutton(PromptGrid,'Text','Cancel','ButtonPushedFcn',@(~,~)SessionEnd);
    uiwait(PromptFig)
...
% Function that creates paired control/microwave violin plots for each axon
    ...
    hold(CurrentTile,'on')
    Significance = cell(1, max(ControlStruct(1).AxonLabels));
    MaxYValues = zeros(1, max(ControlStruct(1).AxonLabels));
    MinYValues = zeros(1, max(ControlStruct(1).AxonLabels));

    % Grabbing parameter values
    for CurrentAxon = 1:max(ControlStruct(1).AxonLabels)
        Parameters.Control = [];
    ...
    Labels = arrayfun(@(x) {sprintf('Axon %d Control',x), sprintf('Axon %d Microwave',x)}, 1:max(ControlStruct(1).AxonLabels), 'UniformOutput',false);
    Labels = [Labels{:}];
    set(gca, 'XTick', sort([(1:1:max(ControlStruct(1).AxonLabels))*2.5-1, (1:1:max(ControlStruct(1).AxonLabels))*2.5]), 'XTickLabel', Labels, 'YGrid', 'on', 'TickDir', 'out');
    ...
    set(gca, 'xlim', [0.5, max(ControlStruct(1).AxonLabels)*2.5 + 0.5]);
    for CurrentAxon = 1:max(ControlStruct(1).AxonLabels)
        ControlX = CurrentAxon*2.5-1;
        MicrowaveX = CurrentAxon*2.5;
...
% Function for creating various segmented statistical figures
    ...
    MaxAxons = max(ControlStruct(1).AxonLabels);
...
% Function to get average APs for a particular axon
    ...
    for n = 1:length(DataStruct)
        ...
        for k = 1:length(APsofInterest)
            ...
            while TempOnsetIndex < 0 || TempAHPIndex > length(TraceTime)
